...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // 🚫 DESHABILITAR TEMPORALMENTE: Sistema de usuarios
  const disabled503 = (_req: any, res: any) => res.status(503).json({ error: "Customer features disabled", code: "CUSTOMER_API_DISABLED" });
  app.all(["/api/auth/phone/register", "/api/auth/phone/login", "/api/auth/login", "/api/auth/register"], disabled503);
  app.use("/api/customer", disabled503);
...
app.get("/api/auth/me", async (req, res) => {
    try {
      if (req.session.adminId) {
        ...
        return res.json({ role: "admin", id: admin.id, email: admin.email, name: admin.name, lastLogin: admin.lastLogin });
      }
      return res.status(503).json({ error: "Sistema de usuarios temporalmente deshabilitado. Solo administradores pueden acceder.", code: "USER_AUTH_DISABLED" });
    } catch (e) {
